...
// --------------------------------------------------------------------------
// ARDrone::getWifiIntesity()
// Description  : Get current Wi-Fi intensity of AR.Drone.
// Return value : Wi-fi percentage [%]
// --------------------------------------------------------------------------
int ARDrone::getWifiIntesity(void)
{
    // Get the data
    if (mutexNavdata) pthread_mutex_lock(mutexNavdata);
...
// --------------------------------------------------------------------------
// ARDrone::EmergencyState()
// Description  : Ckeck if emergency state or not.
// Return value : YES:1 NO:0
// --------------------------------------------------------------------------
int ARDrone::emergencyState(void)
{
    // Get the data
    if (mutexCommand) pthread_mutex_lock(mutexNavdata);
    int emergencyState = (navdata.ardrone_state & ARDRONE_EMERGENCY_MASK) ? 1 : 0;
