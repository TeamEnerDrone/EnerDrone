...
public:
	...
	//Wifi Intensity [%]
	virtual int getWifiIntesity(void);
	...
	//Follow the red line
	virtual bool findCircle (IplImage* image);
	virtual bool findLine(const cv::Mat  imageBottom, double&  offsetPix, double&  angleDeg, const double principalPointU);  
	virtual bool followLine (IplImage* image);

	//Face detection
	virtual bool detectFaces(IplImage *img);

    // Others
	virtual int  emergencyState (void);				// Ckeck if emergency or not
